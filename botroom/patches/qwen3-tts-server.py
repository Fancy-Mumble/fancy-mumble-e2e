# ---------------------------------------------------------------------------
# botroom patch of ghcr.io/malaiwah/qwen3-tts-server's /app/server.py.
#
# Two changes, both marked "botroom:" below, both about a request that should
# not be running any more:
#
#  1. A client that disconnects mid-stream used to leave the CUDA generation
#     thread running to completion ("thread_task.cancel()" cancels the asyncio
#     wrapper, not the thread), while the inference semaphore was released and
#     the next request started on top of it. Two generations on one GPU run at
#     half speed each; a bot cut off mid-sentence made every later sentence
#     crawl. Now the thread is told to stop, and the semaphore is only released
#     once it has.
#
#  2. Generation had no length bound but the model's own 2048 tokens (about
#     170 s of speech). Once in a few hundred sentences the model never emits
#     its stop token and babbles until then; every request behind it waits
#     minutes. `max_new_tokens` is now scaled to the text.
#
# Mounted over /app/server.py by botroom/docker-compose.yml. Regenerate from
# the image if the image is upgraded: `docker cp <container>:/app/server.py`.
# ---------------------------------------------------------------------------
...
def _token_budget(text: str) -> int:
    """botroom: how many codec tokens a text may take, generously.

    Speech runs at about 12 tokens/s and 12-15 characters/s, so roughly one
    token per character; allow two and a half times that plus five seconds,
    capped at the model's own limit. A sentence that legitimately needs more
    than that does not exist; a generation that wants more is a runaway.
    """
    return max(60, min(2048, int(len(text) * 2.5) + 60))
...
def _generate_audio_sync(text: str, voice_id: str, instruct: str | None, language: str) -> tuple:
    ...
    kwargs: dict = {
        ...
        "max_new_tokens": _token_budget(text),  # botroom
    }
    if instruct:
...
def _wav_to_bytes_sync(wav: np.ndarray, sr: int, fmt: str) -> bytes:
    ...
    # faster-qwen3-tts returns float32 audio; wave.open expects int16 PCM.
    ...
    **Requires ``faster-qwen3-tts``** (tier 1 backend).  Provides the lowest
    ...
    async def generate_pcm() -> AsyncGenerator[bytes, None]:
        async with _INFER_SEM:
            ...
            stop = threading.Event()  # botroom: set when the client is gone
            ...
            def _stream_in_thread() -> None:
                ...
                try:
                    kwargs: dict = {
                        ...
                        "max_new_tokens": _token_budget(text_in),  # botroom
                    }
                    if p.get("instruct"):
                    ...
                    if record.ref_audio_path is not None:
                        ...
                        kwargs["voice_clone_prompt"] = [record.prompt_item]
                    gen = _model.generate_voice_clone_streaming(**kwargs)
                    try:
                        for item in gen:
                            if stop.is_set():  # botroom: nobody is listening
                                break
            ...
            try:
                ...
                # botroom: stop the generation and wait for it, so that the
                # inference semaphore is not released onto a GPU still busy
                # with this request. One step of the generator is ~80 ms.
                stop.set()
                try:
                    await asyncio.wait_for(asyncio.shield(thread_task), timeout=5.0)
