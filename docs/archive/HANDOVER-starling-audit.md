...
A throwaway `run-per-file.sh` (a scratch copy, since gone) did: start Starling
on 64738 → murmur shim on spare ports → one
...
`signal-pchat` 2/2 all pass against the fork.
