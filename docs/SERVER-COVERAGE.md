...
| Pchat message/fetch/ack/offline drain | Covered/partial | pchat replay, reconnect and message-loss tests cover the happy path. Fetch windows are now covered in `starling-pchat` unit tests: both directions, the two walks agreeing on the archive, a forward page that catches up, the count charged once, a both-ends cursor refused and a throttled fetch refused out loud. Empty history, the remaining ack statuses and offline queue drain are still missing, and no *client-driven* paging test exists yet |
...
| Pchat server-managed channels (`pchat_protocol = 3`) | Covered/partial | `starling` e2e covers a late joiner reading the whole archive and the plaintext being absent from every file the deployment wrote; unit tests cover sealing, the AAD binding, a wrong key dropping rows and a missing key refusing to store. A client-side test needs the client half, which is not committed yet |
