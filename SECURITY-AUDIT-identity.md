...
Stated because a list of findings is not a description of the system.
...
The broader point is the one in the opening: four instances of one mistake, and
the most recent found by accident. A regression test naming the *class* — that
an identity is never derived from a single unverified signal — would be worth
more than any individual fix here.
